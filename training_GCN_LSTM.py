...
def train_model(model, train_loader, val_loader, optimizer, scheduler, criterion, num_epochs=10, patience=10, output_dir="outputs"):
    ...
    model_save_path = os.path.join(output_dir, "gcn_lstm_model.pth")
...
if __name__ == "__main__":
    ...
    # Initialize model
    model = GCN_LSTM(node_feature_dim, sequence_feature_dim, gcn_out_channels,lstm_hidden_dim, lstm_layers).to(device)
    ...
    save_train_losses_path = os.path.join(output_dir, "GCN_LSTM_train_losses.pkl")
    save_val_losses_path = os.path.join(output_dir, "GCN_LSTM_val_losses.pkl")
    import pickle
    # 保存到文件
